...
export class PlinkoGame {
   ...
   public async buildBoard(rows: number) {
      ...
      if (this.manager.camera instanceof THREE.OrthographicCamera) {
         ...
         // Plus extra space for multiplier zone at bottom
         const boardTop = 10;  // Ball spawn at y=9, add padding
         const boardBottom = 8 - (rows - 1) * 1.4 - 3; // Bottom pegs + multiplier zone + padding
         //   const boardHeight = boardTop - boardBottom;

         // Width: pegs span from -rows/2*1.4 to +rows/2*1.4
         const boardWidth = rows * 1.4 + 4; // Add padding on sides

         // Set orthographic bounds explicitly
         cam.left = -boardWidth / 2;
         cam.right = boardWidth / 2;
         cam.bottom = boardBottom;
         cam.top = boardTop;
         cam.zoom = 1;

         // Position camera at center of view (Z doesn't matter for orthographic)
         cam.position.set(0, (boardTop + boardBottom) / 2, 10);
         cam.lookAt(0, 0, 0);
